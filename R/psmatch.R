...
#'
#' Perform propensity score matching as described in Lu (2005)
#' "Propensity Score Matching with Time-Dependent Covariates".  Given a longitudinal data frame with covariate
...
#' treated) based on time-dependent propensity scores from a Cox proportional hazards model. If
...
coxph_match <- function(n_pairs = 10^10,
  ...
  matches <- pivot_longer(matches,c("trt.id","con.id"),names_to = "type",values_to = "id")
...
#'   default to all columns except those named by \code{id, time, trt_time}.
#'
#' @return a data frame with matched pairs based on propensity scores generated using a Cox proportional hazards model.  
#' This data frame will have three columns.  \code{trt.id} refers to the treated id, \code{con.id} refers to the id that is
#'   a possible control, \code{distance} refers to the scaled euclidean distance generated based on the propensity scores.
#'
#' @examples
#' df <- data.frame(
#'   hhidpn = rep(1:3, each = 3),
...
covariates = NULL) {
  ...
  data[, covariates] <-
    data[, covariates] %>% mutate_if(is.numeric, scale)
  ...
  model <-
    coxph(Surv(start, time, trt) ~ . - id - trt_time - iid, data = data.cox)
  ...
  nbp.distance2 <- distancematrix(nbp.distance1)
  nbp <- nonbimatch(nbp.distance2)
